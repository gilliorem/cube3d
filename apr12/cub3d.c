...
void	store_color_attributes(t_color *color, char id, char *color_value)
{
	if (id == 'F')
	{
		color->id = 'F';
...
//
int	parse_color(t_file *file, t_config *config, char color_id)
{
	if (!check_color(file, config, color_id))
		return 0;
	if (get_color_id(file))
}
...
int main(int argc, char *argv[])
{
	...
	//if (!check_color_attributes(file, &config.floor, &config.ceiling))
	//	return (0);
	print_color(&config.floor, &config.ceiling);
	return 0;
