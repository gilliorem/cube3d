...
void	get_file_lines(t_file *file)
{
	...
	file->lines = ft_calloc(file->height, sizeof(char *));
	file->fd = try_open_file(file->name);
	i = 0;
	while (1)
	{
		...
		while (file->cur_line[j])
			j++;
		file->lines[i] = ft_strdup(file->cur_line);
		free(file->cur_line);
		i++;
	}
	close(file->fd);
...
{
	// re use above function to split the functions responsabilities
	return (1);
}

int	get_map_size()
{
	// we can get it while parsing
	// we can make 2 full passes in the parser:
	// one that will check the config and get the *map size*
	// 	second that will check the map
}

void	init_map(t_map *map, char *map_line)
{

	//re use what is just bellow.
