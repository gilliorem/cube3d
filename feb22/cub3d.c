...
int	keep_biggest_count(t_file *file)
{
	int	biggest_count;

	if (file->line_count_color >= file->line_count_texture)
		biggest_count = file->line_count_color;
	else
		biggest_count = file->line_count_texture;
	return (biggest_count);
}

int	check_map_existence(t_file *file)
{
	int	flag;
	int	i;
	int	j;
	...
	flag = 0;
	j = 0;
	map_start_line = keep_biggest_count(file);
	file->fd = try_open_file(file->name);
	while (j < map_start_line)
	{
		file->cur_line = get_next_line(file->fd);
		j++;
	}
	...
	while (1)
	{
		...
		if (!file->cur_line)
			break;
		if (flag == 0 && file->cur_line[0] != '\n')
		{
			flag = 1;
			print_debug(">map has started", -1, NULL);
		}
		//print_debug(">", -1, file->cur_line);
		//printf("%s", file->cur_line);
		while (file->cur_line[i] != '\n')
		{
			if (!ft_strchr(" 10NSEW", file->cur_line[i]))
			{
				//TODO: Fix the moment we return; this is leaking... because our gnl keep the stash. We can read untill the end of file and *then*, check
				return (print_debug("Error: map format is wrong ASCII:", file->cur_line[i], NULL) & 0);
			}
			i++;
		}
		file->longest_map_line = i;
		if (i > file->longest_map_line)
			file->longest_map_line = i;
		j++;
		free(file->cur_line);
	}
...
t_map	*init_map(t_file *file)
{
	...
	map->matrix = ft_calloc(map->height, sizeof(int *) + 1);
	while (i < map->height)
	{
		map->matrix[i] = ft_calloc(map->width, sizeof(int) + 1);
		i++;
	}
...
int main(int argc, char *argv[])
{
	...
	if (!check_map_existence(file)) return 0;	
