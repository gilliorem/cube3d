...
int	check_map_existence(t_file *file)
{
	...
	return 0;
		...
		{
			if (file->cur_line[i] != ' ' && file->cur_line[i] != '1' && file->cur_line[i] != '0'
			&& !ft_strchr("NSEW", file->cur_line[i]))
			{
		...
		free(file->cur_line);
	//}
	print_debug(">Last line:", -1, last_line);
	close(file->fd);
	//return (print_debug("couldn't find map in file", -1, 0) & 0);
	return (1);
}
