...
t_map	*init_map(t_file *file)
{
	...
	i = 0;
	j = 0;
	print_debug("xxxxxxxx-INIT MAP-xxxxxxxxx \n", -1, NULL);	
	...
	printf(">MAP BEFORE FIRST LINE:%s", file->cur_line);
	i = 0;
	while (1)
	{
		file->cur_line = get_next_line(file->fd);
	...
	printf("\nxxxxxxxxxMATRIXxxxxxxxxx\n");

	j = 0;
	while (j < map->width) 
	{
		if (!ft_strchr(" 10NSEW", map->matrix[0][j]))
			break;
		printf("%c", map->matrix[0][j]);
		j++;
	}
	return (map);
}
...
int main(int argc, char *argv[])
{
	...
	//get_map_dimension(file, file->map);
