...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/21 08:45:28 by regillio          #+#    #+#             */
/*   Updated: 2026/03/21 09:01:24 by regillio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// switch file to lines
// and at the function call, use file->lines
char	*identify_color_line(t_file *file, char color_id)
{
	int	i;
...
int	check_color_line(char *color_line)
{
	int	i;
	...
	while (color_line[i])
	{
		if (!ft_strchr("FC ,0123456789", color_line[i]))
...
char	**split_color_line(char *color_line)
{
	char	**tmp_color_attributes;
	tmp_color_attributes = ft_split(color_line, ' ');

	return (tmp_color_attributes);
}

int	check_color_id(char *id)
{
	if (id[0] == 'F' || id[0] == 'C')
		return (1);
	return (0);
