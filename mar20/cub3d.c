...
// find the first char of the line
...
// WHY AM I spliting in lines -> I am losing my \n identifier
...
int	check_color_type_id(char *file_content)
{
	...
	if (count != 2)
		return (print_debug("Error. Invalid color types id.", count, NULL) & 0);
	return (1);
}

// 2 loops over file content
...
int	find_color_key(t_file *file, t_color *color, char color_id)
{
	int i = 0;
	int count = 0;
