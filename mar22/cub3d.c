...
int	check_color_line(t_file *file)
{
	char	*color_line[2];
	int	i;
	int	j;

	color_line[0] = identify_color_line(file->lines, 'F');
	color_line[1] = identify_color_line(file->lines, 'C');
	i = 0;
	while (i < 2)
	{
		j = 0;
		while (color_line[i][j])
		{
			if (!ft_strchr("FC ,0123456789", color_line[i][j]))
