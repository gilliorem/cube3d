...
// will return a pointer to the line containing a 'F'
// as we can have multiples F in the file, we can
// 	return a pointer to the first 'F' we find
// 	check for its line_position
// 	we split the file on different lines
// 	we check the first char, if its a 'F' 
// 	we free and return the pointer to file content
// 	we return the pointer

// TODO: check if this function should this be more generic and return a pointer to a poitner of the first line first char and takes a char pointer as an argument
void	split_in_lines(t_file *file) {
...
// check in line chars
// for the lines F C WE SO EA NO

int	check_color_char(char c)
{
	 char color_char[2] = "FC";
	 if (c != color_char[0] && c != color_char[1])
		 return (print_error("wrong color key\n") & 0);
	 return (1);
}

int	split_color_onspace(t_color *color)
{
	int	i;

	i = 0;
	if (!check_color_char(color->line[0]))
	{
		return (0);
	}

	color->line_content = ft_split(color->line, ' ');
...
char	*find_color_line(char **lines, char *needle)
{
	char	*color_line;
	int	i;
	...
	while (lines[i])
	{
		//printf("line[%d]:%s\n",i,lines[i]);
		//printf("line[%d][0]:%c\n",i, lines[i][0]);
		if (is_color_id(lines[i], needle[0]))
		{
			printf("Found color key in line %d\n", i);
			color_line = ft_strnstr(lines[i], needle, ft_strlen(lines[i]));		
			print_debug("Color line:", -1, color_line);
			return (color_line);
		}
		i++;
	}
	return (print_debug("Error, color line format not found", -1, NULL), NULL);
}
...
int main(int argc, char *argv[])
{
	...
	char floor_id[3] = "F ";
	char ceil_id[3] = "C ";
	floor_line = find_color_line(file->lines, floor_id);
