...
void	store_map(t_map *map, t_config *config, t_file *file)
{
	//map matrix
	//player_x, y, player_dir
	char 	**lines;
	int	i;
	int	j;
	...
	while (i < file->height)
	{
		//ft_strlcpy(map->matrix[j], lines[i], ft_strlen(lines[i]));
		ft_memmove(map->matrix[j], lines[i], ft_strlen(lines[i]));
...
//*** we now have 1 + (1 x lines) pointers to free
