...
int	find_map_first_line(t_file *file)
{
	...
	print_debug("last line of the first part:", end_first_part, file->cur_line);
	return 0;
	while (1)
	{
		file->cur_line = get_next_line(file->fd);
		...
		if (start_flag == 0 && is_map_char(file->cur_line[0]))
		{
			start_flag = 1;
			first_line = i;
			print_debug("map is starting at line", first_line, NULL);
		}
		if (bad_char_flag == 0 && start_flag == 1 && !is_map_char(file->cur_line[0]))
		{
			bad_char_flag = 1;
			print_debug("bad char found at line", i, NULL);
		}
		i++;
		free(file->cur_line);
	}
