...
int	check_texture_type_id(t_file *file)
{
	...
	count = 0;
	file->fd = try_open_file(file->name); // why does't the fd reset to 0 ?...
	while (1)
	{
		file->cur_line = get_next_line(file->fd);
		if (!file->cur_line)
	...
	else
	{
		...
		while (i < 4)
		{
			if (texture_type_id[i] != true)
				print_debug("Missing texture type id:",i,NULL);
			i++;
		}
...
int	check_map_existence(t_file *file)
{
	...
	while (1)
	{
		...
		if (!file->cur_line)
			break;
		if (file->cur_line[0] == ' ' || file->cur_line[0] == '1')
		{
			flag = 1;
		}
		free(file->cur_line);
	}
	//print_debug("last_line:", -1, last_line);
	if (flag == 1)
		return (print_debug("map has started", -1, NULL) & 1);
	else
